    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          ...
          box(
            title = "Uji Homogenitas Varians", status = "primary", solidHeader = TRUE, width = 6,
            selectInput("homogeneity_var", "Pilih Variabel Numerik:",
              choices = NULL
            ),
            selectInput("group_var_homogeneity", "Pilih Variabel Pengelompokan (Kategorik):",
              choices = NULL
            ),
            ...
            fluidRow(
              column(
                ...
                conditionalPanel(
                  ...
                  selectInput("group_var_mean", "Variabel Pengelompokan (Kategorik):",
                    choices = NULL
                  ),
                  helpText("Variabel pengelompokan harus memiliki tepat 2 kategori.")
                ),
                conditionalPanel(
...
tabItem(
  ...
  fluidRow(
    box(
      ...
      selectInput("prop_test_type", "Jenis Uji Proporsi:",
        choices = c(
          "Satu Sampel" = "one_sample_prop",
          "Dua Sampel" = "two_sample_prop"
        )
      ),
      selectInput("prop_var", "Variabel Numerik:",
        choices = NULL
      ),
      
      # UI untuk menampilkan range data dan input threshold
      uiOutput("data_range_ui"),
      numericInput("prop_threshold", "Nilai Threshold:", value = NULL),
      helpText("Data > threshold akan dianggap 'sukses', data ≤ threshold dianggap 'gagal'."),
      
      conditionalPanel(
        condition = "input.prop_test_type == 'one_sample_prop'",
        numericInput("prop_hypothesized", "Proporsi Hipotesis (sukses):", 
                    value = 0.5, min = 0, max = 1, step = 0.01),
        selectInput("prop_alternative", "Hipotesis Alternatif:",
          choices = c(
            "Tidak sama dengan (≠)" = "two.sided",
      ...
      conditionalPanel(
        condition = "input.prop_test_type == 'two_sample_prop'",
        selectInput("group_var_prop", "Variabel Pengelompokan:",
          choices = NULL
        ),
        helpText("Variabel pengelompokan harus memiliki tepat 2 kategori."),
        selectInput("prop_alternative_2", "Hipotesis Alternatif:",
          choices = c(
            "Tidak sama (p1 ≠ p2)" = "two.sided",
    ...
    box(
      title = "Uji Varians", status = "primary", solidHeader = TRUE, width = 6,
      ...
      selectInput("variance_var", "Variabel Numerik:",
        choices = NULL
      ),
      conditionalPanel(
        condition = "input.var_test_type == 'one_var'",
        numericInput("sigma_squared", "Varians Hipotesis (σ²):", value = 1)
      ),
      conditionalPanel(
      ...
      actionButton("test_variance", "Uji Varians", class = "btn-primary"),
  ...
  fluidRow(
    ...
    column(
      6,
      box(
        title = "Interpretasi Uji Varians", status = "warning", solidHeader = TRUE, width = 12,
          ...
          box(
            ...
            fluidRow(
              column(
                ...
                selectInput("anova_response", "Variabel Response (Numerik):",
                  choices = NULL
                ),
